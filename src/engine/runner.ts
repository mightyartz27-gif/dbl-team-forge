...
export class Runner {
  ...
  async generate(locked: number[], pool: number[], priorities: Priority[], onResult: (r: GeneratedTeam) => void, lockedBench: number[] = []): Promise<void> {
    ...
    if (!useWorker || !this.worker) {
      ...
      for (const p of priorities) {
        ...
        const res = generateTeams(this.db, { locked, lockedBench, pool, priorities: [p] });
        for (const r of res) {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      w.postMessage({ type: 'generate', id, locked, lockedBench, pool, priorities } satisfies WorkerIn);
    });
  }
}
